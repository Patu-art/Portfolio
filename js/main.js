...
function setupCustomCursor() {
  ...
  let mouseX = innerWidth / 2, mouseY = innerHeight / 2, ringX = mouseX, ringY = mouseY, raf = 0;
  const animate = () => {
  ...
  addEventListener('mouseleave', () => document.documentElement.classList.remove('cursor-visible'));
  addEventListener('mouseenter', () => document.documentElement.classList.add('cursor-visible'));
  const interactiveSelector = 'a, button, input, textarea, select, label, [role="button"], .project-card, .challenge-card, .magnetic';
  document.addEventListener('mouseover', (event) => { if (event.target.closest?.(interactiveSelector)) ring.classList.add('is-active'); });
...
(async () => {
  ...
  try {
    ...
    setupCustomCursor();
    const modules = ['./navbar.js','./smooth-scroll.js','./animations.js','./comparison.js','./counters.js','./projects.js','./project-carousel.js','./validation.js','./contact.js','./portfolio-upgrade.js'];
    const results = await Promise.allSettled(modules.map((src) => import(src)));
    results.forEach((result,index) => { if (result.status === 'rejected') console.error(`Optional module failed: ${modules[index]}`, result.reason); });
    const path = location.pathname.split('/').pop() || 'index.html';
    document.querySelectorAll('.desktop-nav a,.mobile-menu nav a').forEach((link) => { if (link.getAttribute('href') === path) link.setAttribute('aria-current','page'); });
